...
let filter_tests =
  [
    smooth_test "smooth test with no-change smoothing"
      [| 1.0; 2.0; 3.0 |] 1.0 arr;
    smooth_test "smooth test of <1.0 smoothing" [| 1.0; 1.8; 2.43 |] 0.9
      arr;
    smooth_test "smooth test of 0.0 smoothing" [| 1.0; 0.0; 0.0 |] 0.0
      arr
    (* should be of the same output sound-wise*)
    (* smooth_test "smooth test of negative smoothing value" [| 1.0;
       -1.8; -2.43 |] (-0.9) arr *);
  ]
