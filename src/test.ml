...
let () =
  ...
  test "render canvas" (fun () ->
      ...
      I.YUV420.fill r (I.Pixel.yuv_of_rgb (0xff, 0, 0));
      let img =
        I.CanvasYUV420.make ~x:150 ~y:200 ~width:600 ~height:600 r
      in
  ...
  test "manual gradient" (fun () ->
      ...
      for j = 0 to d - 1 do
        for i = 0 to d - 1 do
          I.YUV420.set_pixel_rgba img i j
            (0xff, 0, 0, (i + j) * 0xff / (2 * d))
        done
      done;
  ...
  test "mean cannot clip" (fun () ->
      (* Ensure that we don't have to clip when computing the mean of
         two pixels in bytes. *)
      for s = 0 to 0xff do
        for t = 0 to 0xff do
          for a = 0 to 0xff do
